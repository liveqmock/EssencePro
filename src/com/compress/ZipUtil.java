...
public class ZipUtil {
    // 介绍：ZIP应该算作归档类的压缩方法。zip压缩要通过zipOutputStream执行write方法将压缩数据写到指定输出流中。
    // 这里要先使用checkedOutputStream指定文件校验算法，通常使用CRC32算法。

    public static final String EXT = ".zip"; // 文件扩展名
    ...
    private static void compress(File srcFile, ZipOutputStream zos, String basePath) throws Exception {
        if (srcFile.isDirectory()) { // 路径为压缩路径。下面有多个文件。
