 ...
 * Comments: Mybatis分页拦截器
 * Author: 
 * Create Date: 2013-4-8 上午11:03:17
 * Modified By: 
 * Modified Date: 
            ...
            if (mappedStatement.getId().matches(pageSqlId)) { // 拦截需要分页的SQL
                ...
                if (parameterObject == null) {
                    ...
                    if (parameterObject instanceof Page) { // 参数就是Page实体
                        ...
                        page.setEntityOrField(true); // 见
                                                     // com.jiayu.his.util.base.Page.entityOrField
                                                     // 注释
                        page.setTotalResult(count);
                        if (!page.isPaging()) {
                            page.setNumPerPage(count);
                        }
                    } else if (parameterObject instanceof Map || "java.util.HashMap".equals(parameterObject.getClass())
                            || "java.util.Map".equals(parameterObject.getClass())) {
                        Map map = (Map) parameterObject;
                        if (map.containsKey("obj")) {
                            parameterObject = map.get("obj");
                            Field pageField = ClassUtil.getFieldByFieldName(parameterObject, "page");
                            if (pageField != null) {
                                page = (Page) ClassUtil.getValueByFieldName(parameterObject, "page");
                                if (page == null)
                                    page = new Page();
                                page.setEntityOrField(false); // 见
                                                              // com.jiayu.his.util.base.Page.entityOrField
                                                              // 注释
                                page.setTotalResult(count);
                                if (!page.isPaging()) {
                                ...
                                ClassUtil.setValueByFieldName(parameterObject, "page", page); // 通过反射，对实体对象设置分页对象
                            } else {
                    ...
                    ClassUtil.setValueByFieldName(boundSql, "sql", pageSql); // 将分页sql语句反射回BoundSql.
                }
            }
        }
        return ivk.proceed();
    ...
    private String generatePageSql(String sql, Page page) {
        if (page != null && StringUtils.isNotEmpty(dialect)) {
            ...
            if ("mysql".equals(dialect)) {
                ...
                pageSql.append(page.getNumPerPage() + page.getCurrentResult());
            }
